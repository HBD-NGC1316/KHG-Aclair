...
os.system("split -n l/2 -a3 --additional-suffix=.txt ./data/last-fm/graph/graph_music_1step_s.txt ./data/last-fm/graph/split_")

## Process only split_xxx.txt files
input_file_list = os.listdir("./data/last-fm/graph")
not_split = []
...
ite_ent_dic = dict()
item_list = set()
with open("./data/last-fm/item_entity/arr_lfb2fb_s.txt", "r") as if_file:
...
max_item = max(item_list) ### should contain the largest item number in arr_lfb2fb
chunk_size = (max_item // 3)+3
# Create a 2D list
list_group_index = [c for c in range(0, 4 * chunk_size, chunk_size)]
...
for ind in range(len(list_group_index)-1):  
    ...
    output_file = "./data/last-fm/graph/item_group_"+str(ascii_lowercase[cnt])+".txt"
    print(output_file)
